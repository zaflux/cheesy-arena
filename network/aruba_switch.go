...
func (sw *ArubaSwitch) GetStatus() string {
	return sw.Status
}
...
func (sw *ArubaSwitch) ConfigureTeamEthernet(teams [6]*model.Team) error {
	...
	addTeamVlan := func(team *model.Team, vlan int) {
		...
		gatewayIp := fmt.Sprintf("10.%s.%d", teamPartialIp, switchTeamGatewayAddress)
		...
		// Usable range: 10.x.y.20 through 10.x.y.199
		addCommands = append(addCommands,
			fmt.Sprintf("dhcp-server pool \"dhcp%d\"", vlan),
			fmt.Sprintf("network %s 255.255.255.0", network),
			fmt.Sprintf("range 10.%s.20 10.%s.199", teamPartialIp, teamPartialIp),
	...
	if hasTeams {
		addCommands = append(addCommands, "exit")
